...
Background:
	...
	And Uploaded SPDX SBOM appears on the All SBOMs page

Scenario: Verify Download Licences option on SBOM list page for SPDX SBOM
	Given User navigates to "All SBOMs" page
	When User Selects SPDX SBOM of interest from the SBOM list
	And User Clicks "Action" button
	Then "Download License Report" Option should be visible

Scenario: User Downloads license information for SPDX SBOM from SBOM list page
	Given User navigates to "All SBOMs" page
	When User Selects SPDX SBOM of interest from the SBOM list
	And User Clicks "Action" button
